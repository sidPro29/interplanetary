...
import com.notifiy.interplanetary.BuildConfig
import com.notifiy.interplanetary.data.model.ItvPlan
import com.notifiy.interplanetary.data.model.ItvPurchase
import com.notifiy.interplanetary.data.model.MembershipLevel
import com.notifiy.interplanetary.data.model.PaymentIntentResponse
...
class StripeRepository @Inject constructor(
    ...
    suspend fun getMembershipLevels(): List<ItvPlan> {
        var wpToken = sessionManager.fetchWpToken()
        
        // If no user token, use admin credentials to fetch plans for public visibility
        if (wpToken.isNullOrEmpty()) {
            try {
                val adminLoginRes = apiService.login(com.notifiy.interplanetary.data.model.LoginRequest(BuildConfig.WP_ADMIN_USER, BuildConfig.WP_ADMIN_PASSWORD))
                wpToken = adminLoginRes.token
            } catch (e: Exception) {
    ...
    private fun getDefaultPlans() = listOf(
        ...
        // Yearly
        ItvPlan("8271", "Basic SD Yearly", 19.99, "EUR", "Yearly", "Basic SD", "SD quality, with ads"),
        ItvPlan("8273", "Standard HD Yearly", 49.99, "EUR", "Yearly", "Standard HD", "High Definition"),
        ItvPlan("8275", "Premium UHD Yearly", 79.99, "EUR", "Yearly", "Premium UHD", "Ultra High Definition")
    )
    ...
    suspend fun confirmPurchase(plan: ItvPlan, paymentIntentId: String): Result<Boolean> {
        ...
        try {
            ...
            val adminLoginRes = try {
                apiService.login(com.notifiy.interplanetary.data.model.LoginRequest(BuildConfig.WP_ADMIN_USER, BuildConfig.WP_ADMIN_PASSWORD))
            } catch (e: Exception) { 
